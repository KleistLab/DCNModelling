...
fs =18
width = 1
hist, edges = np.histogram(data_multi_lifetime_hours['lifetime'], bins = np.arange(0,12,1))
...
fs =18
width = 1
hist, edges = np.histogram(data_single_lifetime_hours['lifetime'], bins = np.arange(0,12,1))
freq = hist / float(hist.sum())
plt.bar(edges[0:-1], freq, width=width, align="edge", color='darkgrey')
#plt.legend(fontsize=14)
plt.xlabel('Lifetime of axonal structures (hours)', fontsize=fs)
...
fs =18
width = 30
hist, edges = np.histogram(data_multi_lifetime_minutes['lifetime'], bins = np.arange(0,631,30))
freq = hist / float(hist.sum())
plt.bar(edges[0:-1], freq, width=width, align="edge", color='darkgrey')
#plt.legend(fontsize=14)
...
# ax.set_xlim([0, 150])
ax.set_ylim([0, 1])
ax.set_xticks(np.arange(0,631,30))
...
fs =18
width = 30
hist, edges = np.histogram(data_single_lifetime_minutes['lifetime'], bins = np.arange(0,631,20))
freq = hist / float(hist.sum())
plt.bar(edges[0:-1], freq, width=width, align="edge", color='darkgrey')
#plt.legend(fontsize=14)
plt.xlabel('Lifetime of axonal structures (minutes)', fontsize=fs)
...
plt.xticks(fontsize=fs)
plt.yticks(fontsize=fs)
plt.text(32,55.2, 'n=' + str(len(data_multi_unique_hours)), fontsize=fs)
ax = plt.gca()
ax.set_xlim([32, 45])
...
plt.xticks(fontsize=fs)
plt.yticks(fontsize=fs)
plt.text(32,105.4, 'n=' + str(len(data_single_unique_hours)), fontsize=fs)
ax = plt.gca()
ax.set_xlim([32, 45])
